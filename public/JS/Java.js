...
async function saveScanToDatabase(categoria, puntos) {
    ...
    try {
        ...
        if (data.success) {
            console.log('Puntos guardados correctamente en BD:', data.message);
        } else {
            console.error('Error al guardar puntos en BD:', data.error);
        }
    } catch (error) {
        console.error('Error de red al intentar guardar los puntos:', error);
    }
}
